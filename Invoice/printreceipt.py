...
MissionAnd    # Reference number (adapted based on payment method)
    y_pos += 20
    payment_method = invoice_data.get('PaymentMethod', 'CASH').upper()
    if payment_method == 'CHEQUE':
        label_text = "Cheque No."
    elif payment_method == 'ONLINE':
        label_text = "Transaction Ref."
    else:  # CASH
        label_text = "Reference No."
    
    draw.text((50, y_pos), label_text, fill=black_color, font=body_font)
    cheque_no = invoice_data.get('OnlineChequeNo', '')
    draw.text((200, y_pos), str(cheque_no), fill=black_color, font=body_font)elismFund: None
